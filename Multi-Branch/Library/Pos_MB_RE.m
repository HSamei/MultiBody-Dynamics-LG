function [POS,POS_R,POS_F,g_EE] = Pos_MB_RE(ROBOT,THETA,NumStep)
...
% Written by BD Bhai
    ...
    %% For First Branch 
    for i = 1:(length(ROBOT) - 5) / 2 - 2                                               %[Body]     Correspond to the i_th Body in the Robot
        if strcmpi(ROBOT{2*i+1}.Type,'RIGID')
            ...
            POS_R(:,i_R) = g(1:3,4); i_R = i_R + 1;                                 %[R3]       Save Position at Beginning of Link, index to next
            g_temp = g;                                                             %[SE(3)]    Save Config. of Branch Root
    ...
    g_EE(:,:,1) = g;            %[SE(3)]    Save EE Transformation
    
    %% For Second Branch
    for i = (length(ROBOT) - 5) / 2 - [1,0]                                         %[Body]     Correspond to the i_th Body in the Robot
        if strcmpi(ROBOT{2*i+3}.Type,'FLEXIBLE')
            g = g_temp * expm3(hat(ROBOT{2*i+2}.Twist * (ROBOT{2*i+2}.HomePos + THETA(i)))); %[SE(3)]    Transformation to After Joint_{i,i-}
            ds = ROBOT{2*i+3}.L / ROBOT{2*i+3}.N;                                   %[m]        Size of Differential Element (used to compute f_prev)
            
            for ii = 1:(ROBOT{2*i+3}.N * NumStep)
                index = ceil(ii/NumStep);                                           %[]         Determine the Local Strain Value at Step
                g = g * expm3(hat(ROBOT{2*i+3}.f_prev(:,index)) * ds/NumStep);      %[SE(3)]    Transformation to s = s + ds/NumStep
                POS(:,iii) = g(1:3,4); iii = iii + 1;                               %[R3]       Save Position at s = s + ds/NumStep, index to next
                POS_F(:,i_F) = g(1:3,4); i_F = i_F + 1;                             %[R3]       Save Position at s = s + ds/NumStep, index to next
            end
            g_temp = g;
    ...
    g_EE(:,:,2) = g;        %[SE(3)]    Save EE Transformation
